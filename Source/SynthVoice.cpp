...
void VibesSynthVoice::prepare (const juce::dsp::ProcessSpec& spec)
{
  ...
  osc3GainSmooth.setCurrentAndTargetValue (*apvts.getRawParameterValue ("osc3Gain"));
                                        ...
                                        int startSample, int numSamples)
{
  ...
  osc3GainSmooth.setTargetValue (*apvts.getRawParameterValue ("osc3Gain"));
  ...
  const bool osc3On = *apvts.getRawParameterValue ("osc3Enabled") >= 0.5f;
  ...
  for (int i = 0; i < numSamples; ++i)
  {
    ...
    float s3 = oscillator3.processSample();
    if (! osc1On) s1 = 0.0f;
    if (! osc2On) s2 = 0.0f;
    if (! osc3On) s3 = 0.0f;
    ...
    const float sample = (s1 * osc1GainSmooth.getNextValue()
                        ...
                        + s3 * osc3GainSmooth.getNextValue())
                       * adsr.getNextSample();
